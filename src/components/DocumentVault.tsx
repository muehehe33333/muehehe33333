...
export default function DocumentVault() {
  ...
  const handleUploadOrEdit = async (e: React.FormEvent) => {
    ...
    if (user) {
      if (editingId) {
        ...
        if (uploadError) { alert('Gagal mengupload: ' + uploadError.message); setUploading(false); return; }
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="bg-slate-900 text-white p-6 sm:p-8 rounded-[2rem] shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-5 overflow-hidden relative">
        <div className="flex items-center gap-5 relative z-10">
          <div className="p-3 sm:p-4 bg-emerald-500/20 text-emerald-400 rounded-2xl backdrop-blur-sm border border-emerald-500/30">
            <ShieldCheck size={32} />
          </div>
          <div>
            <h2 className="text-2xl font-black tracking-tight">Brankas Dokumen</h2>
            <p className="text-emerald-100/70 text-sm mt-1">Penyimpanan terenkripsi untuk berkas vital.</p>
          </div>
        </div>
        <button onClick={() => { resetForm(); setIsFormOpen(true); }} className="w-full md:w-auto relative z-10 bg-emerald-600 text-white font-bold py-3 px-5 rounded-2xl hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/30 text-sm">
          <Plus size={18} /> Upload Berkas
        </button>
          ...
          {documents.length === 0 ? (
            ...
            <div key={doc.id} className="bg-slate-50/80 border border-slate-100 p-5 rounded-[1.5rem] hover:bg-slate-50 hover:shadow-sm transition-all flex flex-col group">
              <div className="flex items-start gap-4 mb-4">
                <div className={`p-3 rounded-2xl shrink-0 border ${getCategoryColor(doc.category)}`}><File size={24} /></div>
                ...
                <div className="flex flex-col gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity shrink-0">
                  <button onClick={() => handleEditClick(doc)} className="p-1.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:text-amber-600 hover:bg-amber-50 hover:border-amber-200 transition-all shadow-sm"><Edit3 size={14} /></button>
                  <button onClick={() => handleDelete(doc.id, doc.file_url)} className="p-1.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-all shadow-sm"><Trash2 size={14} /></button>
                </div>
              </div>
              
              <div className="mt-auto border-t border-slate-200/60 pt-4">
                <a href={doc.file_url} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl hover:text-emerald-600 hover:border-emerald-200 hover:bg-emerald-50 text-xs font-bold transition-all shadow-sm">
                  <ExternalLink size={14} /> Buka / Unduh File
                </a>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-[999] flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl animate-in slide-in-from-bottom-full sm:zoom-in-95 p-7 pb-24 sm:pb-7 max-h-[90vh] overflow-y-auto">
            <form onSubmit={handleUploadOrEdit} className="space-y-5">
              <div className="flex justify-between items-center mb-6 border-b border-slate-50 pb-4">
                <h4 className="font-bold text-base text-slate-800">{editingId ? 'Edit Info Dokumen' : 'Upload Dokumen Baru'}</h4>
              ...
              <div><label className="block text-xs font-bold text-slate-500 mb-1.5 ml-1">Nama Dokumen</label><input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Contoh: KTP Asli" className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-medium focus:ring-2 focus:ring-emerald-100 outline-none" required /></div>
              <div><label className="block text-xs font-bold text-slate-500 mb-1.5 ml-1">Kategori</label><select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-emerald-100 text-slate-700 outline-none">{categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></div>
              ...
              {!editingId && (
                <div>
                  ...
                  <label className="cursor-pointer flex flex-col items-center justify-center border-2 border-dashed border-slate-200 bg-slate-50/50 rounded-[1.5rem] py-8 hover:bg-emerald-50 hover:border-emerald-200 transition-all group">
                    <div className="p-3 bg-white rounded-full shadow-sm mb-3 group-hover:text-emerald-500 transition-colors"><UploadCloud size={24} className="text-slate-400 group-hover:text-emerald-500" /></div>
              ...
              {editingId && (
                <div className="bg-emerald-50 p-4 rounded-xl text-xs text-emerald-700 font-medium border border-emerald-100">
                  File fisik tidak bisa diubah demi keamanan. Hapus dan upload ulang jika ingin mengganti file utamanya.
                </div>
              )}
